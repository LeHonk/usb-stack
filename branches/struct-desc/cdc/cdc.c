...
uint16_t
getaCDC( char *buffer, uint16_t len )
{
	uint16_t pkt_len = (uint16_t) ( data_end - data ); /* TODO: Check ring buffer compability */
...
uint16_t
getsCDC( char *string, uint16_t len )
{
	uint16_t i;

	for ( i = 0; i < len && (*string++ = getcCDC()); i++ ) { 
//		if ('\n' == *buffer || 0x00 == *buffer) break;
	}
	return ++i;
}
...
uint16_t
putaCDC( const char *str, unsigned int len )
{
	...
	while ( len ) {
		while ( BusyCDC (  ) );
		DisableUsbInterrupts (  );
		pkt_len = ( len < CDC_RXTX_BUFFER_SIZE ) ? (unsigned char) len : CDC_RXTX_BUFFER_SIZE;
