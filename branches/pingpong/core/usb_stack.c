/* $Id: usb_stack.c 85 2012-07-13 20:55:21Z tomhol $
...
usb_datahandler_t  class_setup_handler[USB_NUM_INTERFACES];
void *             class_setup_context[USB_NUM_INTERFACES];
usb_datahandler_t  vendor_setup_handler;
void              *vendor_setup_context;
...
BDentry   usb_bdt[2 * USB_ARCH_NUM_EP]          /* No PP */
#elif USB_PP_BUF_MODE == 1
BDentry   usb_bdt[2 * USB_ARCH_NUM_EP + 1]      /* PP on EP0 out */
#elif USB_PP_BUF_MODE == 2
BDentry   usb_bdt[4 * USB_ARCH_NUM_EP - 2]      /* PP on all but EP0 */
#elif USB_PP_BUF_MODE == 3
BDentry   usb_bdt[4 * USB_ARCH_NUM_EP]          /* PP on all */
#endif
#if defined(PIC_24F)
...
#if USB_PP_BUF_MODE == 0 || USB_PP_BUF_MODE == 4
unsigned char usb_ep0_out_buf[USB_EP0_BUFFER_SIZE];
unsigned char usb_ep0_in_buf[USB_EP0_BUFFER_SIZE];
#elif USB_PP_BUF_MODE == 1
unsigned char usb_ep0_out_buf[2][USB_EP0_BUFFER_SIZE];
unsigned char usb_ep0_in_buf[USB_EP0_BUFFER_SIZE];
#elif USB_PP_BUF_MODE == 2
unsigned char usb_ep0_out_buf[2][USB_EP0_BUFFER_SIZE];
unsigned char usb_ep0_in_buf[2][USB_EP0_BUFFER_SIZE];
#endif
#pragma udata
          ...
          int num_strings )
{
	...
	for ( i = 0; i < USB_ARCH_NUM_EP; i++ ) {
		endpoints[i].type = 0;
		endpoints[i].in_DTS = 0;                      /* At first all is DATA0 */
		endpoints[i].in_PP = USB_PP_EVEN;              /* At first all eps are even */
		endpoints[i].out_DTS = 0;                      /* At first all is DATA0 */
		endpoints[i].out_PP = USB_PP_EVEN;              /* At first all eps are even */
		...
		endpoints[i].out_buffer_even = NULL;
		endpoints[i].out_buffer_odd  = NULL;
		endpoints[i].in_buffer_even  = NULL;
		endpoints[i].in_buffer_odd   = NULL;
#endif
...
void
usb_configure_bdt( void )
{
	int i;

	USB_UEP[0] = USB_EP_CONTROL;

/* Configure endpoingt zero */
#if USB_PP_BUFF_MODE == 0 || USB_PP_BUF_MODE == 4
	...
	usb_bdt[USB_CALC_BD( 0, USB_DIR_IN , USB_PP_EVEN )].BDSTAT = DTS;
#elif USB_PP_BUFF_MODE == 1
	usb_bdt[USB_CALC_BD( 0, USB_DIR_OUT, USB_PP_EVEN )].BDCNT = USB_EP0_BUFFER_SIZE;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_OUT, USB_PP_EVEN )].BDADDR = usb_ep0_out_buf_even;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_OUT, USB_PP_EVEN )].BDSTAT = UOWN + DTSEN;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_OUT, USB_PP_ODD  )].BDCNT = USB_EP0_BUFFER_SIZE;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_OUT, USB_PP_ODD  )].BDADDR = usb_ep0_out_buf_odd;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_OUT, USB_PP_ODD  )].BDSTAT = UOWN + DTSEN;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_IN , USB_PP_EVEN )].BDCNT = 0;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_IN , USB_PP_EVEN )].BDADDR = usb_ep0_in_buf;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_IN , USB_PP_EVEN )].BDSTAT = DTS;
#elif USB_PP_BUF_MODE == 2
	...
	usb_bdt[USB_CALC_BD( 0, USB_DIR_OUT, USB_PP_ODD  )].BDSTAT = UOWN + DTSEN;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_IN , USB_PP_EVEN )].BDCNT = 0;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_IN , USB_PP_EVEN )].BDADDR = usb_ep0_in_buf_even;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_IN , USB_PP_EVEN )].BDSTAT = DTS;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_IN , USB_PP_ODD  )].BDCNT = 0;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_IN , USB_PP_ODD  )].BDADDR = usb_ep0_in_buf_odd;
	usb_bdt[USB_CALC_BD( 0, USB_DIR_IN , USB_PP_ODD  )].BDSTAT = DTS;
#endif

	/* Configure endpoints */
	for ( i = 1; i < USB_ARCH_NUM_EP; i++ ) {
...
#elif USB_PP_BUF_MODE == 2 || USB_PP_BUF_MODE == 3
		usb_bdt[USB_CALC_BD( i, USB_DIR_OUT, USB_PP_EVEN )].BDCNT = endpoints[i].buffer_size;
		usb_bdt[USB_CALC_BD( i, USB_DIR_OUT, USB_PP_EVEN )].BDADDR = endpoints[i].out_buffer_even;
		usb_bdt[USB_CALC_BD( i, USB_DIR_OUT, USB_PP_EVEN )].BDSTAT = UOWN + DTSEN;
		usb_bdt[USB_CALC_BD( i, USB_DIR_OUT, USB_PP_ODD  )].BDCNT = endpoints[i].buffer_size;
		usb_bdt[USB_CALC_BD( i, USB_DIR_OUT, USB_PP_ODD  )].BDADDR = endpoints[i].out_buffer_odd;
		usb_bdt[USB_CALC_BD( i, USB_DIR_OUT, USB_PP_ODD  )].BDSTAT = UOWN + DTSEN;
		usb_bdt[USB_CALC_BD( i, USB_DIR_IN , USB_PP_EVEN )].BDCNT = 0;
		usb_bdt[USB_CALC_BD( i, USB_DIR_IN , USB_PP_EVEN )].BDADDR = endpoints[i].in_buffer_even;
		usb_bdt[USB_CALC_BD( i, USB_DIR_IN , USB_PP_EVEN )].BDSTAT = DTS;	// Set DTS => First packet inverts, ie. is Data0
		usb_bdt[USB_CALC_BD( i, USB_DIR_IN , USB_PP_ODD  )].BDCNT = 0;
		usb_bdt[USB_CALC_BD( i, USB_DIR_IN , USB_PP_ODD  )].BDADDR = endpoints[i].in_buffer_odd;
		usb_bdt[USB_CALC_BD( i, USB_DIR_IN , USB_PP_ODD  )].BDSTAT = DTS;	// Set DTS => First packet inverts, ie. is Data0
#endif
	}
}
...
usb_handle_transaction( void )
{
	...
	ep = USB_USTAT2EP( trn_status );
	bdp = &usb_bdt[USB_USTAT2BD( trn_status )];
	endpoints[ep].out_PP = USB_USTAT2PPI( trn_status );
        rbdp = &usb_bdt[USB_CALC_BD( ep,        /* Reply buffer descriptor pointer */
                USB_DIR_IN,                     /* All replies in IN direction */
                endpoints[ep].in_PP )];            /* TODO: Implement Ping-Pong buffering */
...
usb_handle_setup( void )
{
	...
	bdp->BDSTAT =
		( !( bdp-> BDADDR[USB_bmRequestType] & USB_bmRequestType_PhaseMask )
...
usb_handle_in( void )
{
	...
	if ( endpoints[USB_USTAT2EP( trn_status )].in_handler ) {
		endpoints[USB_USTAT2EP( trn_status )].in_handler( endpoints[USB_USTAT2EP( trn_status )].in_context );
	} else {
//              DPRINTF("No handler\n");
	}
}
...
usb_handle_out( void )
{
        int16_t retval;                         /* Liveness warning - IGNORE */
	DPRINTF("Out EP: %u Handler: 0x%p\t", ep, endpoints[ep].out_handler);
	if ( endpoints[ep].out_handler ) {
		retval = rendpoints[ep].out_handler( endpoints[ep].out_context, bdp->BDCNT, bdp->BDADDR );
		if (0 == retval) usb_ack_out( ep );
		if (-1 == retval) usb_unset_endpoint_handler( ep );
	} else {
		DPRINTF("No handler\n");
		usb_stall_endpoint( ep );
	}
}
...
usb_handler_t
usb_register_class_setup_handler( uint8_t iface, usb_datahandler_t handler )
{
...
usb_handler_t
usb_register_vendor_setup_handler( usb_datahandler_t handler, void *context )
{
...
usb_register_endpoint( unsigned int ep, usb_uep_t type,
			...
			usb_datahandler_t out_handler, usb_datahandler_t in_handler, 
			void *out_context,  void *in_context )
{
	...
	endpoints[ep].out_context = out_context;
	endpoints[ep].in_handler = in_handler;
	endpoints[ep].in_context = in_context;
}

void
usb_set_in_handler( uint8_t ep, usb_datahandler_t in_handler, void *context )
{
	endpoints[ep].in_handler = in_handler;
	endpoints[ep].in_context = context;
}

void
usb_set_out_handler( uint8_t ep, usb_datahandler_t out_handler, void *context )
{
	endpoints[ep].out_handler = out_handler;
	endpoints[ep].out_context = context;
}
...
void
usb_ack_out( uint8_t ep )
{
	BDentry *bd = usb_bdt[USB_CALC_BD( ep, USB_DIR_OUT,  endpoints[ep].PPI )];
	bd->BDCNT = endpoints[ep].buffer_size;
	bd->BDSTAT = ( ( endpoints[ep].DATA )? 0 : DTS ) | UOWN | DTSEN;
	endpoints[ep].DATA ^= 1;                /* Toggle DATA for next transaction */
}
...
struct SEND_CONTEXT {
    ...
    uint8_t		*data;
    void		*context;
    usb_datahandler_t	in_handler;
};

int16_t
usb_send( uint8_t ep, uint16_t length,  uint8_t *data, void *context, usb_datahandler_t in_handler )
{
	BDentry  *bdp;
	struct SEND_CONTEXT *send_ctx:
	uint16_t packet_len;

	if (USB_NUM_EP <= ep) return -1;

	send_ctx = (struct SEND_CONTEXT) malloc( sizeof(struct SEND_CONTEXT) );

	bdp = usb_bdt[USB_CALC_BD(ep, USB_DIR_IN, endpoints[ep].in_PPI)];
	packet_length = (length < endpoints[ep].buffer_size)? length : endpoints[ep].buffer_size;
	memcpy( bdp->BDADDR, data, packet_length);

	send_ctx->length = length - packet_length;
	send_ctx->data = data + packet_length;
	send_ctx->context = context;
	send_ctx->in_handler = in_handler;

	usb_set_in_handler( ep, in_handler, send_ctx );
}

struct SEND_DESC_CONTEXT {
    uint16_t	length;
    ROM uint8_t	*data;
}

void usb_send_descriptor( ROM const uint8_t *desc, int length) {
	struct SEND_DESC_CONTEXT *ctx = (struct SEND_DESC_CONTEXT) malloc(sizeof(struct SEND_DESC_CONTEXT));
    	ctx->length = length;
	ctx->data = desc;
	send_descriptor( ctx, USB_EP0_BUFFER_SIZE, ljHEABLJHSDBCjshbacdljchasbc );
}

static int16_t
send_descriptor( void *ctx, uint16_t length, uint8_t *packet; )
{
	unsigned int i;
	BDentry  *bd;
	uint16_t packet_len;

	if ( ctx->length < USB_EP0_BUFFER_SIZE ) {
		packet_len = ctx->length;
		usb_unset_in_handler( 0 );
		usb_send_ctrl_status();         /* Send ZLP in status phase */
	} else {
		packet_len = USB_EP0_BUFFER_SIZE;
	}
	
	bd = &usb_bdt[USB_CALC_BD( 0, USB_DIR_IN,  endpoints[0].in_PPI )];
	DPRINTF( "Send bd: 0x%p dst: 0x%p src: 0x%p len: %u\n", bd, bd->BDADDR, desc_ptr, packet_len );

	//ARCH_memcpy(rbdp->BDADDR, usb_desc_ptr, packet_len);
	for ( i = 0; i < packet_len; i++ ) {
		bd->BDADDR[i] = desc_ptr[i];
	}
	bd->BDCNT = (uint8_t) packet_len;
	bd->BDSTAT = ( ( endpoints[0].DATA )? 0 : DTS ) | UOWN | DTSEN;
	endpoints[ep].DATA ^= 1;                /* Toggle DATA for next transaction */
	desc_ptr += packet_len;
	desc_len -= packet_len;
}
